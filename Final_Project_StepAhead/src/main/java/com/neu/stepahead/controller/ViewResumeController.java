...
public class ViewResumeController {
	...
	public String viewResume(HttpServletRequest request, HttpSession session, ModelMap model,
			HttpServletResponse response, @PathVariable("id") long personId) {
		...
		try {
			...
			if (session != null) {
				...
				if (sUser != null) {
					...
					JobSeeker jobSeeker = seekerDao.getJobSeekerById(personId);
					String fileName = "/Resumes/" + seekerDao.getResumeName(personId);
					String inlineFileName = jobSeeker.getFirstName() + "_" + jobSeeker.getLastName() + "_Resume.pdf";
